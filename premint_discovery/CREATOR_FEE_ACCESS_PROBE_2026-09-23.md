...
Status: ACCESS_PARTIAL; one mint-specific AMM vault inflow and two creator-level claims verified, full per-mint/2+2 attribution pending. This is separate from the prospective 20-case cohort and does not estimate P(Y100).
...
## 2026-09-23 21:09 UTC — mint-specific on-chain fee event

[Pool transaction probe](https://github.com/arascxe/h40-graac-canary/actions/runs/35920360905) confirmed Pump AMM owns pool `5jYdJBnSrtFTmE6C2nVHmPxKjgSdGTAWw4LodTHUbE74`. In [transaction 2pZ8Yy...](https://solscan.io/tx/2pZ8Yy4dAWatL8afGUVZu9BdHNxJhK6243diSYPMbhce6tAwsy1Y5VoVCMrcq6rTtLPJHrzpyXJdVjsrT8hzj29w), at 2026-09-23 21:03:55 UTC, token balances include BP4W and the pool appears in account keys. A Pump AMM `Sell`/fee calculation log and inner SPL `transferChecked` show **316,399 lamports (0.000316399 wrapped SOL)** moved with the pool as authority to creator vault token account `4D6fMFiU22Pk73foTL5dtyu1XjX9nFnRK4YCedxHpdiK`. That vault token account was independently observed as the **source** of two creator claim transfers above. [Mint/pool/vault linkage](https://github.com/arascxe/h40-graac-canary/actions/runs/35920446267) succeeded.

Added the conservative reusable `fee_audit/pool_fee_flow_v1.py` on main. [Real-chain acceptance](https://github.com/arascxe/h40-graac-canary/actions/runs/35920576854) classified that transaction as `VERIFIED_MINT_POOL_CREATOR_VAULT_FLOW` and rejected the identical transaction with a wrong mint. The paired `fee_audit/creator_receipt_v1.py` independently verifies creator-level claims and rejects unrelated wallet inflows. These are narrow read-only proofs, not a complete mint earnings ledger. This observed flow accrued to the vault **after** the two sampled claims, so it was not part of those earlier payouts. No user-controlled wallet earned anything in these probes.

Next source test: bounded transaction sampling for the other three frozen mints, with pool ownership and recipient account proof. To estimate historical mint-specific total, count *all* canonical eligible fee events over a defined window, handle SOL and USDC quotes plus different buy/sell transfer paths, sharing configurations and claims, and avoid calling a sample or current vault balance total earnings. Separate buyer-linked vs self/related flow before proposing a launch based on this pattern.
